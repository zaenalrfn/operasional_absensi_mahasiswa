...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    {
        title: 'Edit',
        href: `/app-versions/${props.appVersion.id}/edit`,
    },
];

const form = useForm({
    version_number: props.appVersion.version_number,
    release_date: new Date(props.appVersion.release_date).toISOString().split('T')[0],
    platform: props.appVersion.platform || '',
    download_url: props.appVersion.download_url || '',
    is_mandatory: Boolean(props.appVersion.is_mandatory),
    release_notes: props.appVersion.release_notes && props.appVersion.release_notes.length > 0 
        ? props.appVersion.release_notes 
        : [''],
});
...
const submit = () => {
    form.put(route('app-versions.update', props.appVersion.id));
};
</script>

<template>
    <Head title="Edit App Version" />
